...
public:
	...
	{ 
		LOGI("transact_getReaders returned error");
		return String16(""); 
	}
	...
	if (reply.readInt32() != 0) 
	{
		LOGI("transact_isCardPresent returned error");
		return false;
	}
	...
	if (reply.readInt32() != 0) 
	{
		LOGI("transact_openBasicChannel returned error");
		return -1;
	}
	...
	if (reply.readInt32() != 0) 
	{
		LOGI("transact_openBasicChannelAid returned error");
		return -1;
	}
	return reply.readInt64();
    }
	...
	if (reply.readInt32() != 0) 
	{
		LOGI("error on transact_openLogicalChannel");
		return -1;
	}
		
	long long int handle = reply.readInt64();
	LOGI(" retrieved handle: %lli", handle);
	return handle;
    }
	...
	if (reply.readInt32() != 0) 
	{
		LOGI("error on transact_transmit");
	...
	if (reply.readInt32() != 0) 
	{
		LOGI("error on transact_closeChannel");
		return;
	}
    }    
	...
	if (reply.readInt32() != 0) 
	{
		LOGI("error on transact_getLastError");
