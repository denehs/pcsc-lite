 ...
 *
 * $Id: readerfactory.c 4901 2010-04-29 07:26:36Z rousseau $
 */
...
LONG RFAddReader(LPSTR lpcReader, int port, LPSTR lpcLibrary, LPSTR lpcDevice)
{
	...
	for (j = 1; j < ucGetData[0]; j++)
	{
		...
		if (i == PCSCLITE_MAX_READERS_CONTEXTS)
		{
			...
			Log1(PCSC_LOG_ERROR, "No more slot");
		...
		*(sReadersContexts[dwContextB])->pFeeds += 1;
		...
		if (rv != SCARD_S_SUCCESS)
		{
			Log1(PCSC_LOG_INFO, "Cannot connect to slot");
...
int RFStartSerialReaders(const char *readerconf)
{
	...
	{
		Log3(PCSC_LOG_ERROR, "wwwwwwwwwwwwwwwwwwwwwwwwwwwww %d %s", i, reader_list[i].pcFriendlyname);
		int j;
